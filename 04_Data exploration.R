...
# Let's take those who arrived up to 7 years old, and put them as 2nd generation migrants
indiv[migratorystatus == '1.5G migrant' & age_arrival <= 7,length(shnro)] # 4,881 cases
indiv[,generation := migratorystatus]
indiv[generation == '1.5G migrant' & age_arrival <= 7]$generation <- '2G migrant'

# Visualization
jpeg(filename="fig.sample.jpeg",width=8,height=6,units='in',res=500)
ggplot(data=indiv[generation != 'Native'],aes(x=generation,fill=broadbackground)) +
  geom_bar(color='black',position='dodge') +
  theme_bw() +
  xlab("") + ylab("Count") + labs(fill='') +
  theme(legend.position = 'top',legend.justification = 'center')
...
hazarddata <- data.table(
  ...
  broadbackground = c(rep(indiv[,levels(broadbackground)[-1]],each=16,times=4)),
